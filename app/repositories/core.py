...
def upsert_collection(conn, coll_id, name, media_count, api_mtime=0):
    conn.execute(
        """
        INSERT INTO collections (id, name, media_count, api_mtime, last_crawl_time)
        VALUES (?, ?, ?, ?, ?)
        ON CONFLICT(id) DO UPDATE SET
            name = excluded.name,
            media_count = excluded.media_count,
            api_mtime = excluded.api_mtime,
            last_crawl_time = excluded.last_crawl_time
    """,
        (coll_id, name, media_count, api_mtime, time.strftime("%Y-%m-%d %H:%M:%S")),
    )
...
def upsert_video(conn, video_data):
    ...
    if is_invalid:
        existing = conn.execute(
            "SELECT * FROM videos WHERE bv_id = ?", (bv,)
        ).fetchone()
...
def save_collection_videos(conn, collection_id, collection_name, processed_videos):
    ...
    for video_data in processed_videos.values():
        ...
        # 检查是否为新视频
        existing = conn.execute(
            "SELECT bv_id FROM videos WHERE bv_id = ?", (bv,)
        ).fetchone()
        if not existing:
            new_count += 1
        ...
        add_video_to_collection(
            conn, bv, collection_id, video_data["三个时间"]["收藏时间"]
        )
    ...
    for old_bv in current_active - new_bv_set:
        ...
        unfav_count += 1
        video = conn.execute(
            "SELECT title FROM videos WHERE bv_id = ?", (old_bv,)
        ).fetchone()
        if video:
...
def save_following_ups(conn, ups_list):
    if not ups_list:
        logging.warning("关注UP主列表为空，跳过更新以保护现有数据")
        return
    new_mids = set()
    for up in ups_list:
        conn.execute(
        ...
        upsert_up(conn, up["ID"], up["昵称"], up["头像"])
        new_mids.add(up["ID"])
    placeholders = ",".join("?" * len(new_mids))
    conn.execute(
        f"DELETE FROM following_ups WHERE mid NOT IN ({placeholders})",
        list(new_mids),
    )
...
def get_collection_media_count(conn, coll_id):
    row = conn.execute(
        "SELECT media_count FROM collections WHERE id = ?", (coll_id,)
    ).fetchone()
...
def get_collection_incremental_info(conn, coll_id):
    row = conn.execute(
        "SELECT media_count, api_mtime FROM collections WHERE id = ?", (coll_id,)
    ).fetchone()
    if row:
        return row["media_count"], row["api_mtime"]
    return None, None
...
def get_all_videos_index(conn, cache_service=None):
    ...
    all_vc = conn.execute("""
        SELECT vc.bv_id, vc.fav_time, vc.unfav_time, vc.is_active,
               c.name as collection_name
        FROM video_collection vc
        JOIN collections c ON vc.collection_id = c.id
    ...
    vc_by_bv = {}
    for r in all_vc:
        vc_by_bv.setdefault(r["bv_id"], []).append(r)

    video_index = {}
    for row in rows:
        bv = row["bv_id"]
        vc_rows = vc_by_bv.get(bv, [])
...
def get_stats(conn):
    ...
    # 基础统计
    stats["total_videos"] = conn.execute("SELECT COUNT(*) as c FROM videos").fetchone()[
        "c"
    ]
    stats["total_collections"] = conn.execute(
        "SELECT COUNT(*) as c FROM collections"
    ).fetchone()["c"]
    stats["total_ups"] = conn.execute(
        "SELECT COUNT(DISTINCT up_mid) as c FROM videos"
    ).fetchone()["c"]
    stats["invalid_videos"] = conn.execute(
        "SELECT COUNT(*) as c FROM videos WHERE is_invalid = 1"
    ).fetchone()["c"]
    stats["following_ups"] = conn.execute(
        "SELECT COUNT(*) as c FROM following_ups"
    ).fetchone()["c"]

    # 每个收藏夹的视频数量
    stats["collection_counts"] = [
        {"name": row["name"], "count": row["video_count"]}
        for row in conn.execute("""
            SELECT c.name, COUNT(vc.bv_id) as video_count
            FROM collections c
    ...
    stats["duration_distribution"] = OrderedDict((b, 0) for b in duration_buckets)
    for row in conn.execute(
        "SELECT duration FROM videos WHERE duration IS NOT NULL"
    ).fetchall():
        dur = row["duration"]
        try:
            parts = dur.split(":")
            ...
            if total_seconds < 60:
                ...
                bucket = ">1小时"
            stats["duration_distribution"][bucket] += 1
...
def migrate_from_json(json_dir, db_path=None):
    ...
    with get_connection(db_path) as conn:
        ...
        for file_name in json_files:
            ...
            # 创建一个确定性的 collection id（基于名称的 MD5 哈希）
            coll_id = int(
                hashlib.md5(collection_name.encode("utf-8")).hexdigest()[:8], 16
            )
